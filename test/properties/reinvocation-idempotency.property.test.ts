...
const observed = async (scenario: Scenario): Promise<{ trace: Trace; deaths: number }> => {
  ...
  return {
    ...
    trace: {
      ...
      // The emissions are ordinals of the run's own sequence; the closure is not an emission
      // and is identified by what it is, so that two closers cannot reach for one id.
      expectedIds: [
        ...Array.from({ length: emitted }, (_unused, ordinal) => envelopeId(runId, ordinal)),
        lifecycleEnvelopeId(runId, 'completed'),
      ],
